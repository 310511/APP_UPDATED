...
app.post('/api/chess/join', async (req, res) => {
  try {
    console.log(`[REST API] 🔍 Join request received - Method: ${req.method}, URL: ${req.url}`);
    console.log(`[REST API] 🔍 Request body:`, JSON.stringify(req.body));
    console.log(`[REST API] 🔍 Request headers:`, JSON.stringify(req.headers));
    
    const userId = req.body.userId || req.headers['x-user-id'] || 'anonymous';
    const { roomCode, roomId } = req.body;

    console.log(`[REST API] 🔍 Parsed: userId=${userId}, roomId=${roomId}, roomCode=${roomCode}`);
    ...
    if (!actualRoomId && roomCode) {
      ...
      if (redisClient && typeof redisClient.isReady === 'function' && redisClient.isReady()) {
        try {
          ...
          console.log(`[REST API] 🔍 Looked up roomId from code ${normalizedCode}: ${actualRoomId}`);
        } catch (redisError) {
    ...
    console.log(`[REST API] 🔍 Looking for room: ${actualRoomId}`);
    console.log(`[REST API] 🔍 Memory cache size: ${roomCache.size}`);
    console.log(`[REST API] 🔍 Memory cache keys: ${Array.from(roomCache.keys()).join(', ')}`);
    ...
    if (redisClient && typeof redisClient.isReady === 'function' && redisClient.isReady()) {
      try {
        ...
        if (roomDataStr) {
          console.log(`[REST API] ✅ Found room in Redis: ${actualRoomId}`);
        }
      } catch (redisError) {
    ...
    if (!roomDataStr) {
      ...
      if (cachedRoom) {
        ...
        console.log(`[REST API] ✅ Found room in memory cache: ${actualRoomId}`);
      } else {
        console.log(`[REST API] ❌ Room NOT found in memory cache: ${actualRoomId}`);
      }
    }
    
    if (!roomDataStr) {
      console.error(`[REST API] ❌ Room not found: ${actualRoomId} (cache size: ${roomCache.size})`);
